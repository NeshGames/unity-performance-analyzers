...
{
    public class UPA2001LinqUsageAnalyzerTests
    {
        private static Task VerifyAsync(string source)
        {
            var test = new CSharpAnalyzerTest<UPA2001LinqUsageAnalyzer, DefaultVerifier>
            {
                TestCode = source,
                ReferenceAssemblies = ReferenceAssemblies.NetStandard.NetStandard20,
            };
            test.TestState.AdditionalReferences.Add(typeof(UnityEngine.MonoBehaviour).Assembly);
            // UPA2001 is disabled by default; enable it the same way a preset would.
            test.TestState.AnalyzerConfigFiles.Add(("/.editorconfig", @"
root = true

[*.cs]
dotnet_diagnostic.UPA2001.severity = warning
"));
            return test.RunAsync();
        }

        // UPA2001 test case 1 — both calls in the chain report, on the method names
        [Fact]
...
class C : MonoBehaviour
{
    ...
    void Update()
    {
        var result = list.{|UPA2001:Where|}(x => x > 0).{|UPA2001:ToList|}();
        _ = result;
    }
}");
        }
...
class C : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // UPA2001 test case 3 — same-named extension on a user type is excluded
        [Fact]
...
class C : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // UPA2001 test case 4 — query syntax compiles into the same Enumerable methods
        [Fact]
...
class C : MonoBehaviour
{
    ...
    void Update()
    {
        var result = from x in list {|UPA2001:select x|};
        _ = result;
    }
}");
        }

        // UPA2001 test case 5 — one-shot/initialization LINQ is acceptable
        [Fact]
...
class C : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            var descriptor = Assert.Single(new UPA2001LinqUsageAnalyzer().SupportedDiagnostics);
